...
public static class NameNotifyManager
{
    ...
    public static void Notify(this PlayerControl pc, string text, float time = 6f, bool overrideAll = false, bool log = true)
    {
        ...
        if (overrideAll || !Notifies.TryGetValue(pc.PlayerId, out var notifies)) Notifies[pc.PlayerId] = new() { { text, expireTS } };
    ...
    public static void ReceiveRPC(MessageReader reader)
    {
        ...
        if (overrideAll || !Notifies.TryGetValue(playerId, out var notifies)) Notifies[playerId] = new() { { text, expireTS } };
        else notifies[text] = expireTS;
