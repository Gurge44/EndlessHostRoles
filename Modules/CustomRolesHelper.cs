...
internal static class CustomRolesHelper
{
    ...
    public static bool CheckAddonConflict(CustomRoles role, PlayerControl pc)
    {
        return role.IsAdditionRole() && (!Main.NeverSpawnTogetherCombos.TryGetValue(OptionItem.CurrentPreset, out Dictionary<CustomRoles, List<CustomRoles>> neverList) || !neverList.TryGetValue(pc.GetCustomRole(), out List<CustomRoles> bannedAddonList) || !bannedAddonList.Contains(role)) && pc.GetCustomRole() is not CustomRoles.GuardianAngelEHR and not CustomRoles.God && !pc.Is(CustomRoles.Madmate) && !pc.Is(CustomRoles.GM) && role is not CustomRoles.Lovers && !pc.Is(CustomRoles.Needy) && (!pc.HasSubRole() || pc.GetCustomSubRoles().Count < Options.NoLimitAddonsNumMax.GetInt()) && (!Options.AddonCanBeSettings.TryGetValue(role, out (OptionItem Imp, OptionItem Neutral, OptionItem Crew, OptionItem Coven) o) || ((o.Imp.GetBool() || !pc.GetCustomRole().IsImpostor()) && (o.Neutral.GetBool() || !pc.GetCustomRole().IsNeutral()) && (o.Crew.GetBool() || !pc.IsCrewmate()) && (o.Coven.GetBool() || !pc.Is(Team.Coven)))) && (!role.IsImpOnlyAddon() || (pc.IsImpostor() && !pc.Is(CustomRoles.DoubleAgent)) || (pc.Is(CustomRoles.Traitor) && Traitor.CanGetImpostorOnlyAddons.GetBool())) && role switch
        {
            ...
            CustomRoles.Introvert when pc.GetCustomSubRoles().Any(x => x is CustomRoles.AntiTP) || pc.GetCustomRole() is CustomRoles.Leery or CustomRoles.Samurai or CustomRoles.Arsonist or CustomRoles.Revolutionist or CustomRoles.Farseer or CustomRoles.Scavenger or CustomRoles.Analyst => false,
            ...
            CustomRoles.BananaMan when pc.Is(CustomRoles.Doppelganger) => false,
            ...
            CustomRoles.Dynamo when pc.Is(CustomRoles.Spurt) || pc.Is(CustomRoles.Flashman) || pc.Is(CustomRoles.Tank) || pc.Is(CustomRoles.Ankylosaurus)=> false,
            CustomRoles.Spurt when pc.GetCustomSubRoles().Any(x => x is CustomRoles.Dynamo or CustomRoles.Swiftclaw or CustomRoles.Giant or CustomRoles.Flashman) || pc.Is(CustomRoles.Tank) || pc.Is(CustomRoles.Ankylosaurus) => false,
            ...
            CustomRoles.Torch when pc.Is(CustomRoles.Decryptor) => false,
            CustomRoles.Torch when pc.Is(CustomRoles.Farseer) => false,
            ...
            CustomRoles.Sunglasses when pc.Is(CustomRoles.Eclipse) => false,
            ...
            CustomRoles.Gravestone when pc.Is(CustomRoles.Innocent) => false,
            ...
            CustomRoles.Flashman or CustomRoles.Giant when pc.GetCustomRole() is CustomRoles.Swooper or CustomRoles.Wraith or CustomRoles.Chameleon or CustomRoles.Alchemist or CustomRoles.Tank or CustomRoles.Ankylosaurus or CustomRoles.Zombie => false,
            ...
            CustomRoles.Physicist when !pc.IsCrewmate() || pc.GetCustomRole().IsDesyncRole() || pc.Is(CustomRoles.TimeMaster) => false,
            ...
            CustomRoles.Fool when pc.Is(CustomRoles.SabotageMaster) || pc.Is(CustomRoles.GuardianAngelEHR) || pc.Is(CustomRoles.Technician) => false,
