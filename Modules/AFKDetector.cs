...
public static class AFKDetector
{
    ...
    {
        if (!EnableDetector.GetBool() || !GameStates.IsInTask || ExileController.Instance || Main.CachedAlivePlayerControls().Count < MinPlayersToActivate.GetInt() || !PlayerData.TryGetValue(pc.PlayerId, out Data data)) return;
        ...
        if (!data.Counted && data.CurrentPhase == Data.Phase.Warning && data.Timer <= 3f && !pc.IsModdedClient())
        {
            ...
            if (Main.CachedAlivePlayerControls().Count / 2 <= NumAFK)
            {
