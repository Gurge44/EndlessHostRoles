...
internal class Ninja : RoleBase
{
    ...
    public override bool OnVanish(PlayerControl pc)
    {
        ...
        if (!IsUndertaker)
        {
            ...
            if (time >= 1f && target.IsAliveWithConditions() && !target.inVent && pc.RpcCheckAndMurder(target, check: true))
            {
    ...
    private void Take(PlayerControl pc)
    {
        if (MarkedPlayer != byte.MaxValue)
        {
            ...
            if (!(!target.IsAliveWithConditions() || target.inVent || !GameStates.IsInTask) && tpSuccess && pc.RpcCheckAndMurder(target))
            {
                MarkedPlayer = byte.MaxValue;
                SendRPC(pc.PlayerId);
                pc.SetKillCooldown(AdjustedDefaultKillCooldown);
