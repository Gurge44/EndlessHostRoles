...
internal class QuizMaster : RoleBase
{
    ...
    private static Question GetQuestion()
    {
        ...
        (IEnumerable<string> WrongAnswers, string CorrectAnswer) answers = index switch
        {
            ...
            6 when abc => (Main.CustomRoleTypesValues.Where(x => x != randomRole.GetCustomRoleTypes()).Shuffle().Take(2).Select(x => Translator.GetString($"{x}")), Translator.GetString($"{randomRole.GetCustomRoleTypes()}")),
            ...
            14 => (Main.RoleTypesValues.Without(RoleTypes.CrewmateGhost).Without(RoleTypes.ImpostorGhost).Where(x => x != randomRole.GetRoleTypes()).Shuffle().Take(2).Select(x => Translator.GetString($"{x}")), Translator.GetString($"{randomRole.GetRoleTypes()}")),
