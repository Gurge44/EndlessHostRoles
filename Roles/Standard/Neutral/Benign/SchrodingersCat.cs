...
internal class SchrodingersCat : RoleBase
{
    ...
    public override bool OnCheckMurderAsTarget(PlayerControl killer, PlayerControl target)
    {
        ...
        hasValue |= CustomRpcSenderExtensions.Notify(ref sender, killer, string.Format(Translator.GetString("SchrodingersCat.Notify.KillerRecruited"), target.GetRealName(), CustomRoles.SchrodingersCat.ToColoredString()), 10f, setName: false);
        hasValue |= CustomRpcSenderExtensions.Notify(ref sender, target, string.Format(Translator.GetString("SchrodingersCat.Notify.RecruitedByKiller"), killer.GetRealName(), killerRole.ToColoredString()), setName: false);
