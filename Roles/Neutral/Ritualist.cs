...
namespace TOHE.Roles.Neutral
{
    public static class Ritualist
    {
        ...
        private static void SendRPC(byte playerId, byte targetId)
        {
            ...
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SetRitualist, SendOption.Reliable);
        ...
        public static void SetRitual(PlayerControl killer, PlayerControl target)
        {
            if (!IsRitual(killer.PlayerId, target.PlayerId))
            {
                ...
                Logger.Info($"{killer.GetNameWithRole().RemoveHtmlTags()}：Reveal target → {target.GetNameWithRole().RemoveHtmlTags()} || Remaining: {RitualCount[killer.PlayerId]} uses", "Ritualist");
