...
public class Pelican : RoleBase
{
    ...
    public static bool CanEat(PlayerControl pc, byte id)
    {
        ...
        return target != null && target.IsAlive() && !target.inVent && !Medic.ProtectList.Contains(target.PlayerId) && !target.Is(CustomRoles.GM) && !target.Is(CustomRoles.Pestilence) && !target.Is(CustomRoles.Fragile) && !IsEaten(pc, id) && !IsEaten(id);
    }
