...
public static class HeadHunter
{
    ...
    {
        float tempkcd = KCD;
        if (Targets.Contains(target.PlayerId)) _ = System.Math.Clamp(KCD -= SuccessKillCooldown.GetFloat(), MinKCD.GetFloat(), MaxKCD.GetFloat());
        else _ = System.Math.Clamp(KCD += FailureKillCooldown.GetFloat(), MinKCD.GetFloat(), MaxKCD.GetFloat());
        if (KCD != tempkcd)
        {
