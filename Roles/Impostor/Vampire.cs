...
namespace EHR.Impostor
{
    public class Vampire : RoleBase
    {
        ...
        public override bool OnCheckMurder(PlayerControl killer, PlayerControl target)
        {
            ...
            if (target.Is(CustomRoles.Opportunist) && target.AllTasksCompleted() && Opportunist.OppoImmuneToAttacksWhenTasksDone.GetBool())
            {
                return true;
            }
            ...
            if (CanKillNormally)
            {
                return killer.CheckDoubleTrigger(target, Bite);
            }
        ...
        {
            if (!AmongUsClient.Instance.AmHost || !GameStates.IsInTask || ExileController.Instance)
            {
                return;
            }

            byte vampireID = vampire.PlayerId;
            if (!IsThisRole(vampire.PlayerId))
            {
                return;
            }
        ...
        {
            if (vampire == null || target == null || target.Data.Disconnected)
            {
                return;
            }
