...
namespace EHR.AddOns.Crewmate
{
    public class Workhorse : IAddon
    {
        ...
        {
            if (!pc.IsAlive() || IsThisRole(pc.PlayerId))
            {
                return false;
            }

            if (pc.Is(CustomRoles.Needy) || pc.Is(CustomRoles.Lazy) || pc.Is(CustomRoles.Bloodlust))
            {
                return false;
            }

            if (pc.GetCustomRole().GetVNRole(true) is CustomRoles.Impostor or CustomRoles.Shapeshifter)
            {
                return false;
            }

            TaskState taskState = pc.GetTaskState();
            if (taskState.CompletedTasksCount + 1 < taskState.AllTasksCount)
            {
                return false;
            }
            ...
            {
                return canBeTarget && pc.Is(CustomRoleTypes.Crewmate);
            }
        ...
        {
            if (!CustomRoles.Workhorse.IsEnable() || PlayerIdList.Count >= CustomRoles.Workhorse.GetCount())
            {
                return false;
            }
            ...
            if (pc.Is(CustomRoles.Snitch) && !OptionSnitchCanBeWorkhorse.GetBool())
            {
                return false;
            }

            if (!IsAssignTarget(pc))
            {
                return false;
            }

            if (IRandom.Instance.Next(100) >= SpawnChance.GetInt())
            {
                return false;
            }
