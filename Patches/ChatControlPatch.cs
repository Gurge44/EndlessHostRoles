...
public class ChatManager
{
    ...
    public static void GetMessage(PlayerControl player, string message)
    {
        ...
        switch (operate)
        {
            case 1 when player.IsAlive():
                Logger.Info($"Special Command: {message}", "ChatManager");
                cancel = true;
                break;
            case 2:
            ...
            case 4:
                ...
                break;
            case 3:
                chatHistory.Add($"{player.PlayerId}: {message}");
    ...
    public static void SendPreviousMessagesToAll()
    {
        ...
        string msg;
        List<CustomRoles> roles = Enum.GetValues(typeof(CustomRoles)).Cast<CustomRoles>().Where(x => x != CustomRoles.KB_Normal).ToList();
        ...
        for (int i = chatHistory.Count; i < 21; i++)
        {
            msg = $"{Translator.GetString("HideMessage")}";
            var player = Main.AllAlivePlayerControls.ToArray()[rd.Next(0, totalAlive)];
    ...
    private static void SendRPC(PlayerControl senderPlayer, string senderMessage)
    {
        ...
        _ = writer.StartRpc(senderPlayer.NetId, (byte)RpcCalls.SendChat)
            .Write(senderMessage)
            .EndRpc();
        _ = writer.EndMessage();
        writer.SendMessage();
