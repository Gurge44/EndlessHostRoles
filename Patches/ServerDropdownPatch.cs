...
public static class ServerDropdownPatch
{
    ...
    public static bool Prefix(ServerDropdown __instance)
    {
        ...
        __instance.firstOption.ChangeButtonText(translationController.GetStringWithDefault(currentRegion.TranslateName, currentRegion.Name));
        ...
        for (var index = 0; index < displayRegions.Count; index++)
        {
            ...
            buttonPool.Text.text = translationController.GetStringWithDefault(regionInfo.TranslateName, regionInfo.Name);
            buttonPool.Text.ForceMeshUpdate();
            buttonPool.Button.OnClick.RemoveAllListeners();
            buttonPool.Button.OnClick.AddListener(() => __instance.ChooseOption(regionInfo));
